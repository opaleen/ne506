...
def log_book(angles, k_eff, k_eff_target):
    ...
    with open(filename, "a", newline="") as csvfile:
        ...
        if write_header:
            headers = (
                ["evaluation"]
                + [f"drum_{i+1}_angle" for i in range(geom.number_of_control_drums)]
                + ["k_eff", "abs_diff"]
            )
        ...
        row = (
            [evaluation_count]
            + list(angles)
            + [k_eff.nominal_value, abs(k_eff.nominal_value - k_eff_target)]
        )
        writer.writerow(row)


def simulate_model(
    drum_rotation_angle_list,
):
...
def genetic_optimization():
    ...
    result = differential_evolution(
        simulate_model, bounds=[[-90, 90]] * geom.number_of_control_drums, workers=1
    )
    print(result)
