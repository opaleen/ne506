...
def parse_arguments():
	...
	parser = argparse.ArgumentParser(description=" Input settings of control drum rotation problem")
	parser.add_argument('--rotation_angle', type=float, default=0, help='rotation angle of the control drums')
	parser.add_argument('--rotation_step', type=float, default=3, help='rotation step in each iteration ')
...
def generate_model (rotation_angle):
    ...
    settings_file.max_lost_particles =500
    ...
    return geometry,materials,settings_file


if __name__ == "__main__":

    rotation_angle = parse_arguments().rotation_angle
    rotation_step = parse_arguments().rotation_step
    # i am gonna try 10 step
    k_eff_history = [1]
    delta =0
    for i in range(10):
